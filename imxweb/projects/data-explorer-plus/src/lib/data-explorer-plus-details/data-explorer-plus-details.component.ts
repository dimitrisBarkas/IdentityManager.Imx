...
export class DataExplorerPlusDetailsComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void>  {
    this.updateTabDataSource(this.lotsOfTabs[0]);
  ...
  public getTabs(configParm: string): void {
    ...
    const getTabsParam = (items: ExplorerItem[]): string | null => {
      for (const item of items) {
        if (item.ConfigParm === configParm) {
          ...
          const result = getTabsParam(item.Children);
          if (result) return result; 
        }
      }
      return null; // Return null if not found at all
    };
    getTabsParam(this.dataSourcedynamic);
  } 
  ...
  updateTabDataSource(tabLabel: string): void {
